...
static inline void
witness_assert_not_lockless(witness_tsdn_t *witness_tsdn) {
	if (!config_debug || witness_tsdn_null(witness_tsdn)) {
		return;
	}

	witness_list_t *witnesses = &witness_tsdn_tsd(witness_tsdn)->witnesses;
	unsigned d = witness_depth_to_rank(witnesses, WITNESS_RANK_MIN);

	if (d == 0) {
		witness_depth_error(witnesses, WITNESS_RANK_MIN, 1);
	}
}
