...
edata_can_acquire_neighbor(edata_t *edata, rtree_contents_t contents,
	...
	/* It's not safe to access *neighbor yet; must verify state first. */
	...
	if (edata_pai_get(edata) != edata_pai_get(neighbor)) {
		return false;
	}
...
emap_try_acquire_edata_neighbor_impl(tsdn_t *tsdn, emap_t *emap, edata_t *edata,
	...
	rtree_leaf_elm_t *elm = rtree_leaf_elm_lookup(tsdn, &emap->rtree,
	    rtree_ctx, (uintptr_t)neighbor_addr, false, false);
	if (elm == NULL) {
		return NULL;
	}
	...
	if (!edata_neighbor_head_state_mergeable(edata_is_head_get(edata),
	    neighbor_contents.metadata.is_head, forward)) {
		return NULL;
	}
...
emap_deregister_boundary(tsdn_t *tsdn, emap_t *emap, edata_t *edata) {
	/*
	 * The edata is either in an active state, or protected by ecache locks.
	 * This means even if other threads discover the edata pointer through
	 * rtree lookup (for coalesce), they will not attempt to access it
	 * because the state will not match.
	 */
